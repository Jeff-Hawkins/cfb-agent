...
# Load Streamlit Cloud secrets FIRST before anything else
...
if analyze_btn:
    if home_team == away_team:
        ...
        if line_check["cnt"].iloc[0] == 0:
            st.warning(f"⚠️ No historical betting line found for {home_team} vs {away_team} in {season}.")
        ...
        try:
            ...
            st.error(f"⚠️ Analysis failed — one or both teams may not have enough data in the {season} season.")
