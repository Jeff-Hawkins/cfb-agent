...
# ---------------------------------------------------------------------------
# v2 — 15-feature model (season-1 lookups, clipped outliers, portal added)
# ---------------------------------------------------------------------------
...
def _build_features_v2(
    ...
    h_new  = _scalar_v2(coa, "school", "year", "is_new",         home_team, ly, default=0)
    a_new  = _scalar_v2(coa, "school", "year", "is_new",         away_team, ly, default=0)
    h_wpct = _scalar_v2(coa, "school", "year", "career_win_pct", home_team, ly, default=0.5)
    a_wpct = _scalar_v2(coa, "school", "year", "career_win_pct", away_team, ly, default=0.5)
...
def predict_win_probability_v2(
    ...
    Args:
        ...
        season: Season year (default 2025). Lookups use season-1.
        neutral_site: True for neutral-site games.

    Returns:
        dict with keys ``win_prob`` (calibrated) and ``raw_win_prob``,
        or a plain string error message if SP+ data is missing.
    """
    _saved = os.path.join(os.path.dirname(__file__), "saved")
    model_path  = os.path.join(_saved, "win_prob_model_v2.pkl")
    cols_path   = os.path.join(_saved, "feature_cols_v2.pkl")
    scaler_path = os.path.join(_saved, "platt_scaler_v2.joblib")
...
def predict_win_probability_batch_v2(games: list, season: int) -> list:
    ...
    Args:
        ...
        season: Season year. Lookups use season-1.

    Returns:
        List of input dicts enriched with home_win_prob, away_win_prob,
        raw_home_win_prob. Games with missing SP+ data are silently dropped.
    """
    import logging
    logger = logging.getLogger(__name__)
    ...
    for game in games:
        ...
        if features is None:
            logger.warning(
                "No v2 data for %s vs %s in %s — skipping",
                game["home_team"], game["away_team"], season,
            )
            continue
        feature_rows.append(features)
